# AI Agent Context — keralavotemap.site
...
`proj=keralavotemap.site`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=keralavotemap.site` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=keralavotemap.site` | Start the dev server, skipping install. |
| `make test proj=keralavotemap.site` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=keralavotemap.site
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy keralavotemap.site"` runs `gh repo create` and
...
- **Live URL:** https://keralavotemap.site/  *(update once first deploy succeeds)*
...
- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:keralavotemap.site` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://keralavotemap.site/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status keralavotemap.site` resolves cleanly).
...
`portfolio project check keralavotemap.site` (run from `sites/portfolio/`).
